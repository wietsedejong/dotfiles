...
if [ ! -d ~/dotfiles ];then
    if gum confirm "DO YOU WANT TO START THE INSTALLATION OF THE wdots DOTFILES NOW?" ;then
    ...
    if gum confirm "DO YOU WANT TO START THE UPDATE OF YOUR wdots DOTFILES NOW?" ;then
        echo ":: Update started."
    elif [ $? -eq 130 ]; then
            exit 130
    else
